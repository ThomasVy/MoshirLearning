...
public class CreateCoursePage extends Page {
	...
	public CreateCoursePage(ProfessorGUI professorGUI, ArrayList<Course> courses, HomePage homePage) {
		...
		enter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textField_1.getText().length() != 0) {
					...
					if (approved == false) {
						JOptionPane.showMessageDialog(null, "Invalid input for new course.",
								"Failed to Create New Course", JOptionPane.ERROR_MESSAGE);
					} else {
					...
					JOptionPane.showMessageDialog(null, "Invalid name.", "Failed to Create New Course",
		...
		// Random random = new Random();
		// int newId = 10000000 + random.nextInt(90000000); // Need to fix id generator
		// Course newCourse = new Course(newId, professorGUI.getProfessor().getId(),
		// textField.getText(), false);
		// boolean approved = (boolean) professorGUI.sendToClient(newCourse,
		// "CreateNewCourse");
		// if (approved == false) {
		// JOptionPane.showMessageDialog(null, "Invalid input for new course.", "Failed
		// to Create New Course", JOptionPane.ERROR_MESSAGE);
		// } else {
		// professorGUI.getCourses().add(newCourse);
		// professorGUI.createAllPages();
		// professorGUI.refreshPages();
		// CreateCoursePage.this.setVisible(false);
		// professorGUI.getCoursePages().get(professorGUI.getCourses().size() -
		// 1).setVisible(true);
		// }
		// } else {
		// JOptionPane.showMessageDialog(null, "Invalid name.", "Failed to Create New
		// Course", JOptionPane.ERROR_MESSAGE);
		// }
		// }
		// });
